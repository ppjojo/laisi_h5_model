<template>
    <div class="app-container">
        <div class="container-edit">
            <el-button type="primary" size="mini" @click="btn_add()" v-if="checkPer('add')">添加配置项</el-button>
        </div>
        <el-table v-loading="loading" :data="list" element-loading-text="Loading" border fit highlight-current-row
            ...
            <el-table-column align="center" prop="testTypeName" label="测试类型名称" width="150">
            </el-table-column>
            <el-table-column align="center" prop="testItemList" label="适用的测试项集合">
            ...
            <el-form :model="form" label-width="100px" :rules="rules" ref="form">
                ...
                <el-form-item label="测试类型" prop="testTypeId">
                    <el-select v-model="form.testTypeId" clearable>
                        <el-option v-for="item in testTypeList" :key="item.id" :label="item.testTypeName"
                            :value="item.id">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="测试项" prop="testItemIdList">
                    <div v-for="(item, ind) in form.testItemList" :key='ind' style="margin-bottom: 15px;">
                        <el-select v-model="item.testItemId" @change="testItemsListShow">
                            <el-option v-for="tItem in allTestItemList" v-show="!tItem.disabled || tItem.id==item.id"
                                :label="tItem.testItemName" :value="tItem.id"></el-option>
                        </el-select>
                        <el-button v-if="ind == form.testItemList.length-1" icon="el-icon-plus" size='small' circle
                            @click="addTestItem"></el-button>
                        <el-button type="danger" size='small' icon="el-icon-delete" circle
                            @click="delTestItem(ind, item)">
                        </el-button>
                    </div>
                    <el-button v-if=" !(form.testItemList && form.testItemList.length)" icon="el-icon-plus" size='small'
                        circle @click="addTestItem"></el-button>
...
<script>
    import { listItem, addItem } from '@/api/factoryTesting/testItemConfiguration'
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    testItemNameList: [
                        { type: 'array', required: true, message: '请至少选择一个测试项', trigger: 'change' },
                    ],
                },
                projectList: [],
                allTestItemList: [],
