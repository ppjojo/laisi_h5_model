<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="list" element-loading-text="Loading" border fit highlight-current-row
			...
			<el-table-column align="center" prop="wake_count" label="醒来次数">
			</el-table-column>
			<el-table-column align="center" prop="wake_time" label="醒来时长(秒)">
        <template slot-scope="scope">
                    {{scope.row.wake_time/1000}}
        </template>
			</el-table-column>
			<el-table-column align="center" prop="deepTime" label="深睡眠时长(秒)">
        <template slot-scope="scope">
                    {{scope.row.deepTime/1000}}
        </template>
			</el-table-column>
			<el-table-column align="center" prop="lightTime" label="浅睡眠时长(秒)">
        <template slot-scope="scope">
                    {{scope.row.lightTime/1000}}
        </template>
			</el-table-column>
...
<script>
    import { memberList, backgroundSleepSearch } from '@/api/dataSearch/dataSearch'
    ...
    export default {
        ...
        methods: {
            ...
            searchData() {
                ...
                backgroundSleepSearch(para).then(res => {
                    this.loading = false;
                    if (!res.data) return;
