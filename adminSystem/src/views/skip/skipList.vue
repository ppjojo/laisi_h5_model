        ...
        <el-table v-loading="loading" :data="list" element-loading-text="Loading" border fit highlight-current-row
            ...
            <el-table-column prop="mode" align="center" label="模式">
                <template scope="scope">
                    {{scope.row.mode == 2?"倒计时跳":scope.row.mode == 3?"倒计数跳":""}}
                </template>
            </el-table-column>
            ...
            <el-form :model="form" :rules="rules" ref="form" label-width="100px">
                ...
                <el-form-item label="比赛模式：">
                    <el-select v-model="form.mode" :disabled="dialogTitle=='编辑'" clearable>
                        <el-option v-for="item in matchModelList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                    <el-select v-if="form.mode==2" :disabled="dialogTitle=='编辑'" v-model="form.modeValue" clearable
                        filterable>
                        <el-option v-for="item in limitNumList_counttime" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                    <el-input-number v-if="form.mode==3" :disabled="dialogTitle=='编辑'" v-model="form.modeValue"
                        controls-position="right"  :min="50" :max="9999"></el-input-number>
                </el-form-item>

                <el-form-item label="比赛次数：">
                    <el-select v-model="form.matchTimesType" clearable :disabled="dialogTitle=='编辑'"
                        @change='selectMatchTimesChange'>
                        <el-option v-for="item in matchTimesList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>

                    <el-select filterable v-show="form.repeatTimes!= -1" :disabled="dialogTitle=='编辑'"
                        v-model="form.repeatTimes" clearable>
                ...
                <el-form-item label="比赛类型：">
                    <el-select v-model="form.type" :disabled="dialogTitle=='编辑'" clearable
                        @change='selectMatchTypeChange'>
                        <el-option v-for="item in matchTypeList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>

                    <el-select v-show="form.type=='team'" :disabled="dialogTitle=='编辑'" v-model="form.teamPersonLimit"
                        clearable filterable>
                        <el-option v-for="item in limitNumList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                ...
                <el-form-item label="团队数：" v-if="form.type=='multiTeam'" prop="teamNum">
                    <el-input-number v-model="form.teamNum" controls-position="right" @change="handleChange" :min="2"
                        :max="52"></el-input-number>
                </el-form-item>
                <!-- 团队数，上线 52  2-52-->
                <el-form-item label="团队人数：" v-if="form.type=='multiTeam'" prop="teamPersonLimit">
                    <el-input-number v-model="form.teamPersonLimit" controls-position="right" @change="handleChange"
                        :min="5" :max="50"></el-input-number>
                </el-form-item>
                <!-- 人数，5 — 50 -->

                <el-form-item label="比赛邀请码：">
                    <el-select v-model="form.isVerify" clearable @change='selectMatchNatureChange'>
                        <el-option v-for="item in matchNatureList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                ...
                <el-form-item label="比赛时间：">
                    <el-date-picker v-model="timeRange" type="datetimerange" start-placeholder="开始时间"
                        @change="datetimerangeChange" end-placeholder="结束时间" value-format="timestamp">
                    </el-date-picker>
                </el-form-item>
        ...
        <el-dialog title="赛况详情" :visible.sync="joinVisible" width="70%">
            <el-form :model="form" :rules="rules" ref="form" label-width="100px">
                ...
                <el-form-item label="比赛模式：">
                    ...
                    <el-select v-if="form.mode==2" disabled v-model="form.modeValue" 
                        filterable>
                        <el-option v-for="item in limitNumList_counttime" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                    <el-input-number v-if="form.mode==3" disabled v-model="form.modeValue"
                        controls-position="right" :min="50" :max="999"></el-input-number>
                </el-form-item>

                <el-form-item label="比赛次数：">
                    <el-select v-model="form.matchTimesType" disabled
                        @change='selectMatchTimesChange'>
                        <el-option v-for="item in matchTimesList" :key="item.value" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>

                    <el-select filterable v-show="form.repeatTimes!= -1" disabled
                ...
                <el-form-item label="团队数：" v-if="form.type=='multiTeam'" prop="teamNum">
                    <el-input-number v-model="form.teamNum" controls-position="right" @change="handleChange" :min="2" disabled
                        :max="52"></el-input-number>
                </el-form-item>
                <!-- 团队数，上线 52  2-52-->
                <el-form-item label="团队人数：" v-if="form.type=='multiTeam'" prop="teamPersonLimit">
                    <el-input-number v-model="form.teamPersonLimit" controls-position="right" @change="handleChange" disabled
                        :min="5" :max="50"></el-input-number>
                </el-form-item>
                <!-- 人数，5 — 50 -->
                ...
                <el-form-item label="比赛时间：">
                    <el-date-picker v-model="timeRange" type="datetimerange" start-placeholder="开始时间" disabled
                        @change="datetimerangeChange" end-placeholder="结束时间" value-format="timestamp">
                    </el-date-picker>
                </el-form-item>

                <el-form-item label="比赛图片：" prop="picUrl">
                    <el-upload ref='upload' action="" :http-request="requestFile" :show-file-list="false" disabled
                        class="avatar-uploader">
                        <img v-if="form.picUrl" :src="form.picUrl" class="avatar">
                        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                    </el-upload>
            ...
            <el-divider content-position="left"><svg-icon icon-class="salesDept_date"/> 参与用户</el-divider>
            <div class="container-search">
                <el-form :inline="true" :model="joinSearchForm">
                    <el-form-item label="报名人ID：">
                        <el-input v-model.trim="joinSearchForm.competitorId"
                                placeholder="请输入报名人ID/账号"
                                clearable></el-input>
                    </el-form-item>
                    <el-form-item label="报名人昵称：">
                        <el-input v-model.trim="joinSearchForm.nickName"
                                placeholder="请输入报名人昵称"
                                clearable></el-input>
                    </el-form-item>
            ...
            <el-table :data="joinList" border fit highlight-current-row size="small ">
                ...
                <el-table-column align="center" prop="repeatNumber" label="跳绳次数">
                </el-table-column>
...
<script>
    ...
    import { listItem, addItem, updateItem, deleteItem, detailItem, joinItem } from '@/api/skip/skip'
