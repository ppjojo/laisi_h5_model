    ...
    <div class="containBox">
      ...
      <div class="sportBox">
        <div class="dateBox">
          ...
          <span class="chooseDate icon iconfont icon-tongyong-rili " @click="dateshow = isShare?false:true"></span>
        </div>
        <div class="personBox">
          <ul>
...
<script>
var vm;
import { getGroupInfo, joinGroup, applyOnly } from "@a/groupIndex";
const defaultSettings = require("../settings.js");
import { NavBar, Icon, NoticeBar, Popup, DatetimePicker, Toast } from "vant";
...
export default {
  ...
  methods: {
    initData() {
      ...
      if (
        parseInt(this.$store.state.searchTime / 1000) ==
      ...
      if (this.isGrouptMember) {
        ...
        if (this.groupItem.isInviteConfirm) {
          applyOnly({
            ...
            if (res.code == 0) {
              this.$interaction
                .appNative("LSTH5APP_ApplyJoinGroup", {
                  groupInviteId: res.data.id + "",
                  groupId: this.groupId + "",
                  groupOwnerId: this.ownerUserId,
                  groupName: this.groupItem.name,
                  invitedUserId: userId,
                  invitedUserName: JSON.parse(localStorage.getItem("appInfo"))
                    .nickname,
                })
                .then(() => {
                  Toast("已提交审核，等待组长确认");
