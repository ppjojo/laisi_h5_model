<template>
  <div id="app" class="multChangePage" v-cloak>
    ...
    <div class="outside" v-if="flag==1||flag==4">
      <van-field v-model="text" class="club-input" :rows="flag==4?2:3" autosize type="textarea" :maxlength="maxlength" :placeholder="flag==1?'请填写小组口号':'请填写小组名称'" show-word-limit />
    </div>
    <div class="outside" v-else>
      <van-field v-model="text" v-if="isCurrentUser" @input="textChange" class="club-input" rows="10" autosize type="textarea" maxlength="500" placeholder="请填写公告1-500字" show-word-limit />
...
<script>
import { NavBar, Icon, Field, Toast, Dialog } from "vant";
import { groupSettingInfo } from "@a/groupIndex";
import { textReview } from "@u/tool";
...
export default {
  ...
  watch: {
    text(val, oldVal) {
      //普通的watch监听
      this.flag == 1
        ? (this.groupItem.slogon = val)
        : this.flag == 2
        ? (this.groupItem.content = val)
        : (this.groupItem.name = val);

      this.same = this.oldtext == val;
    },
  },
  mounted() {},
  created() {
