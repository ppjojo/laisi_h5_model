<template>
  <div id="app" v-cloak>
    <div class="header">
      <van-nav-bar :title="titleText" @click-left="onclickLeft" @click-right="onClickRight" left-arrow safe-area-inset-top fixed>
        <template #left>
...
<script>
import { createGroup, upDateGroup, checkGroupName } from "@a/createGroup";
import { groupSettingInfo } from "@a/groupIndex";
import { pictureReview, textReview } from "@u/tool";
const defaultSettings = require("../settings.js");
import { NavBar, Uploader, Button, Loading, Field, Toast, Icon } from "vant";
...
export default {
  ...
  data() {
    return {
      ...
      labelArray: this.$route.query.labelId
        ? this.$route.query.labelId.split(",")
        : [],
  ...
  methods: {
    fieldFocus() {
      document.getElementsByClassName("van-nav-bar")[0].style.position =
        "absolute";
      document.getElementsByClassName("van-nav-bar")[0].style.top =
        document.documentElement.scrollTop + "px";
    },
    fieldBlur() {
      document.getElementsByClassName("van-nav-bar")[0].style.position =
        "fixed";
      document.getElementsByClassName("van-nav-bar")[0].style.top = "0px";
    },
    onClickRight() {
      if (!this.edit) return;
