  ...
  <div id="app" class="groupMember" v-cloak>
    ...
    <div class="ub ub-ac ub-ad invitebox" v-if="false">
      <div class="tx-c" @click="shareClub('wechat')">
        <img style="width:0.64rem" :src="require('../img/weixin.png')" alt="">
        <div class="">微信好友</div>
      </div>
      <div class="tx-c" @click="shareClub('wxcircle')">
        <img style="width:0.64rem" :src="require('../img/pyq.png')" alt="">
        <div>朋友圈</div>
      </div>
      <div class="tx-c">
        <img style="width:0.64rem" :src="require('../img/qq.png')" alt="" @click="shareClub('qq')">
        <div>QQ好友</div>
      </div>
      <div class="tx-c">
        <img style="width:0.64rem" :src="require('../img/qqzone.png')" alt="" @click="shareClub('qzone')">
        <div>QQ空间</div>
      </div>
    </div>
...
import { getMyFriend, inviteFriend } from "@a/groupMember";
import {
  NavBar,
  Icon,
  Search,
        ...
        .then(() => {
          inviteFriend({
            ...
            if (res.code == 0) {
              ...
              this.$interaction
                .appNative("LSTH5APP_GroupInvite", this.inviteObj)
                .then(() => {
                  Toast("邀请成功！");
                  setTimeout(() => {
                    this.$router.go(-1);
