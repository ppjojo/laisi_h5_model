      ...
      <van-tabs v-model="active" @click="tabsClick">
        <van-tab v-for="tabItem in all" :title="tabItem.title">
          <van-pull-refresh v-model="tabItem.refreshing" :head-height="90" @refresh="onRefresh" class=" monkey-pull-refresh">
            ...
            <van-list v-model="tabItem.loading" v-if="tabItem.list.length>0" :finished="tabItem.finished" finished-text="没有更多了" @load="onLoad" :immediate-check=false>
...
import { userAttendStatus } from "@a/detail";
      ...
      listItemApi(
        ...
        .then((res) => {
          if (res.code == 0) {
            ...
            if (
              this.all[this.active].page + 1 >
              Math.ceil(res.data.total / this.all[this.active].pageSize)
            ) {
              this.all[this.active].finished = true;
            }
            if (this.all[this.active].page == 0) {
              ...
              this.all[this.active].list = this.all[this.active].list.concat(
                res.data.content
              );
            }
          } else if (
            this.all[this.active].page == 0 &&
            parseInt(res.code) == 1000
          ) {
