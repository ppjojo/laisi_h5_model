<template>
  <div id="app" v-cloak>
    ...
    <div class="creatBox" v-if="flag==1">
      <div class="item_box">
        ...
        <div class="pkName_box">
          <van-field v-model="submitform.name" rows="2" type="textarea" maxlength="16" placeholder="请输入比赛名称吧~" show-word-limit />
        </div>
      </div>
      <div v-if="type=='skipping'">
        <div class="item_box">
          ...
          <div v-if="submitform.mode" class="borderTop">
            <van-cell class="pk_cell" :class="form.pkMode.indexOf('请选择')!=-1?'':'chosed'" @click="modeChange(submitform.mode)" :title="submitform.mode==2?'倒计时长':'倒计数跳个数'" is-link :value="form.pkMode" />
          </div>
        </div>
        <div class="item_box">
          <div class="laber">
          ...
          <div v-if="submitform.type=='team'" class="borderTop">
            <van-cell class="pk_cell" :class="form.pkType.toString().indexOf('请选择')!=-1?'':'chosed'" @click="typeChange(submitform.type)" title="团队赛人数" is-link :value="form.pkType" />
          </div>
        </div>
      </div>
      ...
      <div v-else-if="type=='steps'">
        <div class="item_box">
          <div>
            <van-cell class="pk_cell2" :class="form.pkMode.indexOf('请选择')!=-1?'':'chosed'" @click="distanceShow = true" title="比赛模式(公里数)" is-link :value="form.pkMode" />
          </div>
        </div>
        <div class="item_box">
          <div>
      ...
      <div v-else-if="type=='wristball'">
        <div class="item_box">
          <div>
            <van-cell class="pk_cell2" :class="form.pkMode.indexOf('请选择')!=-1?'':'chosed'" @click="countTimeShow = true" title="比赛模式(倒计时转)" is-link :value="form.pkMode" />
          </div>
        </div>
        <!-- <div class="item_box">
                    <div class="laber">
      ...
      <div v-if="type=='wheel'">
        <div class="item_box">
          ...
          <div v-if="submitform.mode" class="borderTop">
            <van-cell class="pk_cell" :class="form.pkMode.indexOf('请选择')!=-1?'':'chosed'" @click="modeChange(submitform.mode)" :title="submitform.mode==2?'倒计时长':'倒计数次数'" is-link :value="form.pkMode" />
          </div>
        </div>
        <div class="item_box">
          <div>
    ...
    <div class="creatBox" v-if="flag==2">
      <div class="item_box">
        <div class="pkRepeatTimes">
          <van-cell class="pk_cell2" :class="form.pkRepeatTimes.indexOf('请选择')!=-1?'':'chosed'" @click="playTimesShow = true" title="比赛次数" is-link :value="form.pkRepeatTimes" />
        </div>
      </div>
      <div class="item_box">
        <div class="laber">
      ...
      <div class="item_box">
        <div>
          <van-cell class="pk_cell2" :class="form.pkStart.indexOf('请选择')!=-1?'':'chosed'" @click="startTimeShow = true" title="开始时间" is-link :value="form.pkStart" />
        </div>
      </div>
      <div class="item_box">
        <div>
          <van-cell class="pk_cell2" :class="form.pkEnd.indexOf('请选择')!=-1?'':'chosed'" @click="endTimeShow = true" title="结束时间" is-link :value="form.pkEnd" />
        </div>
      </div>
      <div class="item_box" v-if="type!='steps'">
        ...
        <div class="pkName_box" v-if="form.pkGameReward==2">
          <van-field v-model="submitform.gameReward" rows="2" type="textarea" maxlength="20" placeholder="请输入比赛奖励吧~" show-word-limit />
        </div>
      </div>
    ...
    <van-popup v-model="countTimeShow" position="bottom" round>
      <van-picker confirm-button-text="确定" :title="type=='skipping'?'倒计时跳时长':type=='wristball'?'倒计时转时长':'倒计时时长'" show-toolbar :columns="countTimeColumns" :default-index="1" @cancel="countTimeShow = false" @confirm="onCountTimeConfirm" />
    </van-popup>

    <van-popup v-model="distanceShow" position="bottom" round>
      <van-picker title="公里数" confirm-button-text="确定" show-toolbar :columns="distanceColumns" :default-index="1" @cancel="distanceShow = false" @confirm="onDistanceConfirm" />
    </van-popup>
    ...
    <van-popup v-model="teamShow" position="bottom" round>
      <van-picker confirm-button-text="确定" title="团体赛人数" show-toolbar :columns="teamColumns" @cancel="teamShow = false" @confirm="onteamConfirm" />
    </van-popup>
    <van-popup v-model="playTimesShow" position="bottom" round>
      <van-picker confirm-button-text="确定" show-toolbar title="比赛次数" :columns="playTimesColumns" :default-index="2" @cancel="playTimesShow = false" @confirm="onplayTimesConfirm" />
    </van-popup>
    <van-popup v-model="startTimeShow" position="bottom" round>
      <van-picker confirm-button-text="确定" show-toolbar title="开始时间" :columns="startTimeColumns" @cancel="startTimeShow = false" @confirm="onstartTimeConfirm" @change="onstartTimeChange" />
    </van-popup>

    <van-popup v-model="endTimeShow" position="bottom" round>
      <van-picker confirm-button-text="确定" show-toolbar title="结束时间" :columns="endTimeColumns" @cancel="endTimeShow = false" @confirm="onendTimeConfirm" @change="onendTimeChange" />
    </van-popup>

    <van-dialog v-model="countNumberShow" confirmButtonText="确定" width="80%" :title="type=='skipping'?'倒计数跳个数':'倒计次次数'" show-cancel-button @confirm="onCountNumberConfirm">
      <div>
...
import { textReview, DateTime } from "@u/tool";

import { createCompetition } from "@a/index";
...
export default {
  ...
  methods: {
    ...
    skippingCheckIsNull() {
      ...
      if (!(submitform.mode && submitform.type && submitform.modeValue)) {
        //submitform.name &&
        this.canNext1 = false;
        return;
      } else {
    ...
    runningCheckIsNull() {
      let submitform = this.submitform;
      if (submitform.type && submitform.modeValue) {
        //submitform.name &&
        this.canNext1 = true;
      }
      if (
    ...
    modeChange(name) {
      ...
      if (name == 2) {
        ...
        this.form.pkMode =
          this.type == "skipping" ? "请选择倒计数跳个数" : "请选择倒计次次数";
      }
    },
    typeChange(name) {
      //比赛类型切换回调
    ...
    isVerifyChange(name) {
      ...
      if (name == "true") {
        this.submitform.invitationCode = Math.random()
          .toString(36)
          .substr(2, 4);
    ...
    onstartTimeConfirm(value, index) {
      if (value[0] == "立即开始") {
        ...
        this.initEndTime(
          DateTime.sjc2time("ymdhms", DateTime.getSjc(nowTime) + 3600000)
        );
      } else {
        this.form.pkStart = DateTime.sjc2time(
          "mdhm2",
          DateTime.getSjc(value[0] + " " + value[1].text)
        );

        this.submitform.startTime = DateTime.getSjc(
    ...
    submitFormData() {
      ...
      this.submitform.promoterId = JSON.parse(
