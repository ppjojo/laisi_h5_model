...
import { getSportByMonth } from "@a/api";
export default {
  props: {
    showOnly: {
      //是否只是做显示，点击无效（为true情况是月度统计等）
      type: Boolean,
      default: () => false,
    },
    textTop: {
  ...
  methods: {
    ...
    calindarList(arr, isToday) {
      ...
      for (let i = 0; i < arr.length; i++) {
        if (
          arr[i].otherMonth == "nowMonth" ||
          arr[i].otherMonth == "othermonth"
        ) {
      ...
      getSportByMonth(timeObj).then((res) => {
        ...
        res.data.checkTimeList.forEach((d) => {
          //给上小绿点
          arr.forEach((e) => {
