...
function ajax() {
  ...
  arguments["0"].data=Object.assign({},arguments["0"].data)

  if (getQueryString("isShare") ){
    arguments["0"].data.userId = getQueryString("userId") || "10000";
    ajaxResolve(arguments["0"]);
    return;
  }
...
function ajaxResolve() {
  let requestObject=arguments["0"];
  let token = "";
  let userId = "0";
  if (localStorage.getItem("appInfo")) {
    ...
    arguments["0"].data.timeZone = appInfo.timeZone;
    token = appInfo.token;
    userId = appInfo.userId;
  }

  var ajaxData = {
    type: (requestObject.type || "GET").toUpperCase(),
    url: host + requestObject.url || "",
    async: requestObject.async || "true",
    data: requestObject.data || null,
    dataType: requestObject.dataType || "json",
    contentType:requestObject.contentType ||"application/x-www-form-urlencoded; charset=utf-8",
    beforeSend: requestObject.beforeSend || function () {},
    success:requestObject.success || function () {},
    error: requestObject.error || function () {},
  };
